...
>
> **Last sync**: 2026-06-11 (Claude session — implement fleet: outreach-readiness batch)

> **2026-06-11 changelog #2 (Claude — implement fleet, 5 gated lanes, on branch `fix/scan-citation-label`, UNPUSHED):** Ran a 5-lane agent fleet (each file-disjoint + gated). **Pipeline:** I-06 fixed — `upsert_run` now hard-refuses any `mode=='mock'`/`is_sample` write (code-level, env-proof) + session conftest + `run_eval` clamp clears `GOBLIN_SUPABASE_ENABLED`; competitor acceptance (#22) walgreens→CVS-class recall + junk-domain rejection (+ vertical-hint fallback, flagged inference/verify); term-map expansion node (#16) + Top-5 tracked queries. **pytest 327 pass, eval 3/3, graph-keeper APPROVE.** **Live `runs` table PURGED** — 1,478 mock rows + 51,365 mock recs deleted via MCP; 2 real `live` dogfood runs preserved. **Functions:** Scrapfly WAF bypass hardened (country=us, rendering_wait, 2nd attempt on residential pool) + meijer-class honest-blind-spot lock; npm test 242 pass. **Web (content):** homepage 3-levers section (#18) + 6 learn pages deepened from stubs (#19) + Article/FAQ JSON-LD; integrity APPROVE. **Web (ux/email):** Loader blink cut 1300→420ms (#6); branded welcome email (#9/A-15); 5 tracked queries in scan UI (#17); honest inline scan-email CTA + `/api/scan-email` (#4/#14) — integrity REVISE→fixed (removed false "engineer follows up" promise on the no-mailer path; `queued`→`delivered:false`). **Web (dashboard/auth):** modal login (#10/A-07); 3-step onboarding checklist (#12); tier-gate verified (#11). **Main-thread cleanup:** fixed 9 lint errors from new code (unescaped `<code>` quotes ×7, onboarding setState-in-effect); **removed fabricated "50 prompts × 4 engines"** from the homepage scan teaser (real config: max_queries 8 / teaser 2 — no "50" exists). **web lint 0 + build 0.** Still owner-gated for outreach: secret rotation, DO env (Stripe/Resend/`NEXT_PUBLIC_SITE_URL`), Resend DNS, Supabase SMTP, live $1 e2e, GSC/Bing sitemap submit, live dogfood re-run + approve. Nothing pushed — branch awaits owner review + merge.
...
| A-07 | **Login button → modal on public site** | 🟡 | web | **CODE DONE (branch `fix/scan-citation-label`, unpushed):** `components/auth/LoginModal/` (role=dialog, focus trap, ESC) + `HudLoginTrigger` in HUD; reuses new `lib/useAuthForm.ts` hook; `/login` stays no-JS fallback. Pending merge + live verify |
...
| A-15 | Branded welcome email (magic link) | 🟡 | web/email | **CODE DONE (branch, unpushed):** `sendWelcomeEmail()` in `app/api/webhooks/stripe/route.ts` now sends branded neo-brutalist TABLE HTML (dark/lime/JetBrains Mono, bulletproof CTA) matching `scan-report.html`. Auth/idempotency logic untouched. Pending merge + live-send styling verify (D-10) |
...
| I-07 | DO env for money path: `NEXT_PUBLIC_SITE_URL`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET` (live), `RESEND_API_KEY`, `RESEND_FROM_EMAIL` | ⬜ | infra | Webhook refuses to send links without `NEXT_PUBLIC_SITE_URL`; `.do/app.yaml` documents them as out-of-band SECRETs. Resend click-tracking OFF for auth mail. Apply via `doctl apps update --spec .do/app.yaml` |
| I-09 | **Fix live `BING_INDEXNOW_KEY`** | 🔴 | infra | Live DO env is `031825-6` (garbage leftover) — it passes the route regex and OVERRIDES the real served key `11a3e594ef174a13e7c628f18bba15e2`, so Bing rejects on key-mismatch. Set the env to the real key, or delete it so `/indexnow` serves `public/indexnow.txt`. Blocks all IndexNow submission |
